...
- The DCS AI parent project at `C:\AI_WORKSPACE\AGENTS\STRATEGY_ANALYSIS\DCSAI\` has its own `CLAUDE.md` and `.claude/rules/` enforcing the `src/{util,service,core,output,download}` layout and `dcs-ai-cli` usage rules. Those still apply here.
